...
$email = "sgabion.k12148528@umak.edu.ph";
$password = "12345678";
$password_hash = password_hash($password, PASSWORD_DEFAULT);

$findAccount = $conn->prepare("SELECT * FROM `ACCOUNT` WHERE `Email` = ?");
$findAccount->bind_param("s", $email);
$findAccount->execute();
$result = $findAccount->get_result();

if ($result->num_rows == 0) {
    $insertAccount = $conn->prepare("INSERT INTO `ACCOUNT` (`Username`, `Password`, `Name`, `Email`, `ProfilePicture`, `AccountType`) VALUES (?, ?, ?, ?, ?, ?)");
    $username = "Admin";
    $name = "Admin User";
    $profilePicture = "../resources/images/account-pfp/admin.png";
    $accountType = "Admin";
